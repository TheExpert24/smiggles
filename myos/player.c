...
void play_wav_file_resolved(const char* resolved_path) {
    ...
    while ((bytes_read = (int)syscall_invoke3(SYS_READ, (unsigned int)file_fd, (unsigned int)chunk_buf, (unsigned int)sizeof(chunk_buf))) > 0) {
        ...
        // Optimized UI refresh check: Only update screen character maps when an actual second rolls over
        ...
        // Master pacing clock loop moved here to let disk sectors stay ahead of card drain requirements
        for (uint32_t loop = 0; loop < 240; loop++) {
            outb(0x43, 0x30);
            outb(0x40, 0xA9);
            outb(0x40, 0x04);
            while (1) {
